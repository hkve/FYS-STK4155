...
\section{Method}
    ...
    \subsection{Resampling techniques}
        ...
        \subsubsection*{Cross-validation}
            ...
            \newline\newline
            To reduce the computational cost introduced when increasing the number of folds $k$, we aim to first study the calculated test MSE for $k \in [5,10]$. By studying the stability of the test MSE across different $k$ values, we can identify where the trend stabalizes and used this $k$ for futher studies.   

    \subsection{Expanding to regularised methods}
        \comment{Not married to this section name or separate existence. -\Carl}\\
        With the regularised regression methods, ridge and Lasso, we introduce a hyperparameter $\lambda$ that affects our fit. To explore this, we will study the MSE of the fits using cross validation resampling as the polynomial degree and $\lambda$-value is changed. We will use the same polynomial degrees as when exploring OLS, and look at a wide logarithmically spaced range of $\lambda \in [10^{-9}, 10^1]$. This effectively results in a gridsearch over ($p$,$\lambda$), where the lowest MSE value will give us the optimal set of paramters $(p_{opt}, \lambda_{opt})$.
