...
\section{Theory}
    ...
    \subsubsection{Measure for goodness of fit}
        ...
        \begin{align}
            ...
            \label{eq:theo:r2}
        \end{align}
        \comment{Mabye something about $R^2 \in (-\infty, 1]$, where $R^2 = 1$ is a perfect fit, $R^2 = 0$ is equal to predicting the mean $\bar{y}$ all the time and $R^2 < 0$ is even worse. - \Hakon, agree - \Carl}
...
Something general about initially fitting Franke data using OLS, ridge and lasso. Then moving on to real terrain data something something.
    ...
    And $\epsilon$ is some stochastic noise $\epsilon \distas \normal{0}{\sigma^2}$. By visual inspection, we can fine tune $\sigma$ to create data that looks like noisy digital terrain data. For all fits to the Franke function we used $n = 600$ uniformly distributed points $x,y \distas \mathcal{U}(0,1)$ with a noise level of $\sigma = 0.1$, which is discussed in \ref{app:sec:frankefunction N and sigma}.   

    \subsection{Linear regression}
    \comment{not sure about how we should divide these subsections, but I wanted to write a bit about data scaling - \Carl. Since we compose this design matrix and preform scaling for all models, mabye give this section a  more general name and write about train test splitting -\Hakon}
        ...
        \newline\newline
        \comment{Just wrote down some thoughts, think this should be present in the method section before the resampling part. \Hakon}
        To get a proper assessment of our model, we will split the dataset into a \textit{training set} and \textit{testing set}. The training set is used to fit the model, giving our coefficient estimates $\vec{\hat{\theta}}$. Using the fitted model and the training data, we can calculate metrics to quantify how good our model replicates this data. In the case that our model perfectly predicts the training data $\vec{y}$ based on $X$, the calculated metrics $MSE$ and $R^2$ (eq. \ref{eq:theo:mse}, \ref{eq:theo:r2}) should be 0 and 1 respectively. However, using these scores to assess the goodness of our fit has an underlying problem. Since the model has seen the training data to estimate $\vec{\hat{\theta}}$, we can run into problems where the model begins to fit the $\epsilon$ values and not the (assumed) underlying function $\hat{f}(x)$. This phenomenon is called \textit{overfitting}. To mediate this, we calculate our metrics based on the training data which was not used for fitting. This serves as a way to estimate the errors on data which the model has not seen before. For the artificial data made using the Franke function, we went for a split where $\sfrac{2}{3}$ of the data is used for the training set, while other $\sfrac{1}{3}$ is reserved for the test set.           
...
\section{Results}
    \comment{Proposed structure: OLS, Ridge Lasso, Terrain data \Hakon}
    \subsection{Ordinary least squares}
    We begin by calulating the $MSE$ and $R^2$ score (eq. \ref{eq:theo:mse}, \ref{eq:theo:r2}) fitted to the Franke function for different polynomial degrees. This can be seen in Figure \ref{fig:res:OLS_mse_noresample} and \ref{fig:res:OLS_R2_noresample} respectively. Based on the $MSE$ calculated for the test data, the optimal degree was found to be $p_{opt} = 5$ with $MSE \approx 0.134$. We observe that the train $MSE$ decreases steadily with higher $p$, while the test $MSE$ decreases until we reach $p_{opt}$, after which it increases. The opposite relationship is seen for the $R^2$ score.   
    \begin{figure}[H]
        \centering
    ...
    To investigate the stability of our fits with varying polynomial degrees, we also plot coefficient values with varying polynomial degree. In figure \ref{fig:res:OLS_beta_values_plot} we see these plotted for $p = 1,2,3$ with 95\% confidence intervals. These are calculated using the diagonal $\sigma^2_{\beta,a} = \var{(\hat{\beta})_{aa}}$. Additionally, in figure \ref{fig:res:OLS_beta_values_table} we see the absolute values of these coefficients tabulated for $p \in [1,5]$.
    ...
    \begin{table}[H]
        ...
        10  & 7         & 0.136
        \end{tabular}
        \caption{Showing the optimal degrees $p_{opt}$ and $MSE$ values calculate on the test data using Cross validation with $k = 5, 7$ and $10$.}
        \label{res:tab:OLS_kfold_optimal}
    \end{table}
    
    Just as in the case without resampling, the train $MSE$ decreases with $p$. For the test $MSE$, the increase after the various $p_{opt}$ values is smaller, but the general trend is the same.  
    ...
    \end{figure}
    \subsection{Ridge}
    We now repeat the same analysis for Ridge regression.
...
\section{Discussion}
    ...
    \subsection{OLS}
    Looking back at figure \ref{fig:res:OLS_mse_noresample} and \ref{fig:res:OLS_R2_noresample} showing the $MSE$ and $R^2$ scores calculated using the train and test set, we recognize a common sign of overfitting moving past $p_{opt} = 5$. When moving into this region, we introduce many new degrees of freedom to the fit. This allows our model to accurately recreate the exact points present in the training set, but at the cost of fitting the added noise $\epsilon$ and/or assuming a too complex structure to the underlying function. This can be seen by the large $MSE$ values calculated on the test data, telling us that our model preforms poorly on data which it has not seen.     
