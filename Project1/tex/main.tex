...
\section{Results}
    ...
    \subsection{Ridge and Lasso}
        ...
        \subsubsection{Bias-Variance Decomposition}
            Using the optimal $\lambda$ found from cross-validation heatmap (see Figure~\ref{res:fig:Franke_heatmaps}) we now plot the bias-variance decomposition of Ridge and Lasso as funtions of model complexity. The results can be seen in Figure~\ref{res:fig:bs_bias_var_Ridge_Lasso}. As apposed to OLS, the MSE values of Ridge and Lasso do not increase with polynomial degree after a certain threshold, but rather always decreases (apart from some fluctuations). The plots resemble the OLS for $n=1500$. 
            \begin{figure}[ht]
                \begin{subfigure}{\linewidth}
                    \centering
            ...
            \end{figure}
            This means that there is little to no occurence of overfitting when applying these methods.     
            ...
            Lastly we will examine the bias variance decomposition as a funtion of $\lambda$ (see Figure~\ref{res:fig:bs_bias_var_lmbdas_Ridge_Lasso}).  
            \begin{figure}[ht]
                \begin{subfigure}{\linewidth}
                    \centering
    ...
    This leads us to optimise the regularised methods, seen in Figure~\ref{res:fig:Nica_heatmap}. We find $(p=12, \lambda = 3\cdot 10^{-9})$ as the optimum for Ridge, and $(p=13, \lambda=6 \cdot 10^{-5})$ for Lasso, with respective minimum MSEs $0.1865$ and $0.1887$ -- slightly underperforming OLS.
    ...
    We plot the test prediction of our best fit, OLS with $p=12$, against the test terrain data in Figure~\ref{res:fig:Nica_terrain}.
