...
\section{Method}
    ...
    \subsection{Datasets}
        ...
        \subsubsection{Terrain data}
            ...
            Coordinates SW corner: $13^\circ\,26'\,40''\,\text{N}, 87^\circ\,31'\,40''\,\text{W}$, SE corner: $13^\circ\,26'\,40''\,\text{N}, 87^\circ\,26'\,40''\,\text{W}$ NW corner: $13^\circ\,31'\,40''\,\text{N}, 87^\circ\,31'\,40''\,\text{W}$, NE corner: $13^\circ\,31'\,40''\,\text{N}, 87^\circ\,26'\,40''\,\text{W}$
        ...
        \subsection{Data preparation}
            ...
            To get a proper assessment of our model, we will split the dataset into a \textit{training set} and \textit{testing set}. The training set is used to fit the model, giving our coefficient estimates $\vec{\hat{\theta}}$. Using the fitted model and the training data, we can calculate metrics to quantify how good our model replicates this data. In the case that our model perfectly predicts the training data $\vec{y}$ based on $X$, the calculated metrics MSE and $\Rsquared$ (Eqs.~\ref{theo:eq:mse},~\ref{theo:eq:r2}) should be 0 and 1 respectively. However, using these scores to assess the goodness of our fit has an underlying problem. Since the model has seen the training data to estimate $\vec{\hat{\theta}}$, we can run into problems where the model begins to fit the $\epsilon$ values and not the (assumed) underlying function $f(x)$. This phenomenon is called \textit{overfitting}. To mediate this, we calculate our metrics based on a testing data which was not used for fitting. This serves as a way to estimate the errors on data which the model has not seen before. For the artificial data made using the Franke function, we went for a split where $\sfrac{2}{3}$ of the data is used for the training set, while other $\sfrac{1}{3}$ is reserved for the testing set.
...
\section{Discussion}
    ...
    \subsection{Resampling}
        ...
        The choice of $k=7$ used for our parameter gridsearch is influenced by the computational aspect of the approach. Since a model has to be fitted $k$ times for each set of parameter $(p,\lambda)$, it is clear that this increases the time taken to search the paramter space. However, for both OLS and Ridge, the closed form expressions (eq. \eqref{theo:eq:OLS_coefs} and \eqref{theo:eq:Ridge_coefs} respectively) only require matrix multiplication and inversion. Adding more folds chould potentially be feasible here. However, since Lasso requires a cost function minimization, it is significantly slower than OLS and Ridge, espcially for high $p$. Therefor we get a trade off between the number of $(p,\lambda)$ to try and the number of folds $k$, where $k=7$ was found to allow for high parameter reselution while still being completed in a feasible amount of time.      
...
\begin{appendices}
    ...
    \section{Theoretical proof of concept of the OLS model} \label{app:sec:OLS_proof}
        ...
        The expectatio value of a datapoint $y_i$ is\footnote{In this appendix, we understand repeated indices to be summed over.}
        \begin{align}
