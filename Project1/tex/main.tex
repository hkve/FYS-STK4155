...
\section{Theory}
    ...
    \subsubsection{Measure for goodness of fit}
        ...
        \begin{align}
            ...
            \label{eq:theo:mse}
        \end{align}
        and is a strictly positive measure, that we want to minimise.
        ...
        where $\bar{y} = \frac{1}{n}\sum_{i=1}^n y_i$ is the sample mean of the observed data. This metric falls on the interval $(-\infty, 1]$, where 1 denotes the perfect fit, 0 corresponds to a constant prediction of $\bar{y}$, and negative values corresponds to worse prediction than this.
...
\section{Results}
    ...
    \subsection{Ordinary least squares}
    We begin by calulating the $MSE$ and $R^2$ score (eq. \ref{eq:theo:mse}, \ref{eq:theo:r2}) fitted to the Franke function for different polynomial degrees. This can be seen in Figure \ref{fig:res:OLS_mse_noresample} and \ref{fig:res:OLS_R2_noresample} respectively. 
    \begin{figure}[H]
        \centering
