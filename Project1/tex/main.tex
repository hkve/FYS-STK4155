...
\section{Theory}
    ...
    \subsubsection{Measure for goodness of fit}
        ...
        \begin{align}
            ...
            \label{eq:theo:r2}
        \end{align}
        \comment{Mabye something about $R^2 \in (-\infty, 1]$, where $R^2 = 1$ is a perfect fit, $R^2 = 0$ is equal to predicting the mean $\bar{y}$ all the time and $R^2 < 0$ is even worse. - \Håkon, agree - \Carl}
    ...
    \subsection{LASSO regression}
        ...
        \end{align}
        which as we can see is not differentiable at $\theta_i = 0$. This means that the optimisation of this the probability $P(\vec{\theta}|X, \vec{y})$ is not necessarily possible analytically without putting bounds on the data $X, \vec{y}$, and even numerical algorithms like gradient descent will encounter trouble. Furthermore, it is not optimisable point by point, unless bounds like $X^TX$ being diagonal are enforced, which makes LASSO generally less used. Nevertheless, we can coax a cost function out of the probability that is minimalisable. Letting the cost function be written $\msub{C}{lasso}(\vec{\theta}) = -A\log\pclosed{P(\vec{\theta|X,\vec{y}})} + B$  we can choose the constants $A, B$ such that
        \begin{align}
            \boxed{
...
Something general about initially fitting Franke data using OLS, ridge and lasso. Then moving on to real terrain data something something.
    ...
    \comment{not sure about how we should divide these subsections, but I wanted to write a bit about data scaling - CM. Since we compose this design matrix and preform scaling for all models, mabye give this section a  more general name and write about train test splitting}
        ...
        \newline\newline
        To get a propper assesement of our model, we will split the dataset into a \textit{training set} and \textit{testing set}. The training set is used to fit the actual model, giving our coefficient estimates $\vec{\hat{\theta}}$. The training set is used to evaluate the metrics $MSE$ and $R^2$ scores on data which was not used in fitting the model. This is done to avoid \textit{overfitting}, where our model begings to fit the noise of the dataset, instead of the assumption of the underlying function $\hat{f}(x)$.   
