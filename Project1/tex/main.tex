...
\section{Theory}
    \subsection{Statistical introduction to linear regression}
        ...
        \subsection{Ridge regression and Lasso}
            ...
            Least Absolute Shrinkage and Selection Operation (LASSO) regression builds on the same ideas as ridge regression, by assumes rather a Laplace distribution $L(0, \tau)$ for the parameters $\vec{\theta}$. This gives
            \begin{align}
                P(\vec{\theta}) = \prod_{i=0}^{p-1} \frac{1}{2\tau} \exp{-\frac{|\theta_i|}{\tau}},
            \end{align}
            with a derivative
            \begin{align}
                \d[\theta_k]{P(\vec{\theta})} = \prod_{i=0}^{p-1} -\frac{1}{\tau} \sgn(\theta_k) \frac{1}{2\tau} \exp{-\frac{|\theta_i|}{\tau}} = -\frac{1}{\tau} \sgn(\theta_k) P(\vec{\theta}).
            \end{align}
            The condition for the maximal probability is now
            \begin{align}
                \pclosed{ \frac{1}{\sigma^2}X^T(\vec{y}-X\vec{\theta}) - \frac{1}{\tau} \sgn(\vec{\theta}) } P(\vec{y}|X, \vec{\theta}) P(\vec{\theta}) = 0,
            \end{align}
            which unlike the previous cases cannot be solved point by point in $\vec{\theta}$. Substituting in $\lambda = \sigma^2/\tau$, we find the optimal parameters $\msub{\vec{\hat{\theta}}}{lasso}$ from
            \begin{align}
                \boxed{
                X^TX \msub{\vec{\hat{\theta}}}{lasso} + \lambda\sgn(\msub{\vec{\hat{\theta}}}{lasso}) = X^T\vec{y}
                }
            \end{align}
