...
\section{Method}
    ...
    \subsection{Application on Real Data}
        ...
        Finally we will apply the three regression methods on our Nicaraguan terrain data. We repeat the same analyses as done earlier, optimising the hyperparameters of the regularied methods and finding suitable polynomial expansions to fit the data. We do bias-variance decomposition of the the models as they fit the test-portion of the data, varying the complexity of the design matrix from expansion up to power $p=1\text{--}15$ (\comment{insert correct power here}).
