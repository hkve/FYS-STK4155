...
\section{Method}
    ...
    \subsection{Resampling techniques}
        ...
        \subsubsection*{Bootstrap}
            ...
            {theo:eq:bias and var}). This will create a bias value and variance value for each of the input data points. When evaluating a model of a certain polynomial degree, we find an appropriate measure of the bias and variance for the \textit{entire} model would be the mean across data points. 
    ...
    \subsection{OLS Revisited}
        As concluded in the section above, resampling the data yields a more precise evaluation of our model. We will therefore revisit the implementation of OLS, now employing resampling techniques. 
        \subsubsection{Bootstrap}
...
\section{Results}
    \subsection{Ordinary Least Squares}
        ...
        \comment{I  have written what is supposed to be objective above the figures and subjective below (in case we want to move it to 'discussion') -\Anna}
            As we can see from the plot in Figure~\ref{res:fig:bs_train_test_ols} the bootstrapped models tested on the training data yield more or less identical MSE values and decrease as the model complexity increases. However this is not the case when implementing the testing data. After a certain polynomial degree ($\sim 6$) the MSE value not only increases (as also seen in Figure~\ref{fig:res:OLS_mse_noresample}), but there is a large variance in results of the bootstrapped models. $\epsilon$.  
            \begin{figure}[ht]
                \centering
            ...
            \end{figure}
            This is an indication that the model is overfitted to the testing data; it is adapted to the noice, 
  
            When looking at the bias-variance decomposition of the MSE value (Figure~\ref{res:fig:bs_OLS_bias_variance_tradeoff_ols}), it is evident that when summing the decomposition it results in something very close to the original MSE. The bias of the models starts high and decreases with increasing model complexity. The variance on the other hand is initially $0$ and increases with the polynomial degree. 
            \begin{figure}[ht]
                \centering
            ...
            \end{figure}
            That the bias remains low for a small number of data points corresponds with the notion that there requires a little complex model to approximate few data points. It also makes sense that the variance and MSE quickly increase with the model complexity; the model is easily overfitted. 

            For $n=1500$ the number of datapoints impedes the model from overfitting as such low polynomial degrees. Had we further increased the model complexity, we would again see the signs of overfitting. 
