...
\subsection{Creating a Neural Network and Regression Problems}
    ...
    \subsubsection{Implementing Regularisation}
        Penalising large weights can hinder them from growing arbitrarily large and therefore prevents overfitting. We added the regularisation hyperparameter to the gradients of the weights. If we had implemented a stopping critera, we would also have included it in the cost function. 

        Firstly we chose the best network from the previous explorations: \network{5}{200}, with sigmoid and $\eta = 0.001$. We employd penalisations, $\lambda$, in the range $10^{-9}$ to $10^{-1}$ to this network and plotted the validation MSE as a function of these $\lambda$s.
