...
\subsection{Classification problems}
    ...
    In this work we will consider the famous Wisconsin Breast Cancer data set \cite{Dua:2019}. Tumors are characterized as malignant (M) or benign (B), with 30 attributes such as radius, texture and area computed from digitized images. With $n = 569$ instances present in the data set, it serves a good real-world benchmarking set. We wish to classify each tumor as either M or B, based on the tumors various attributes. These categories are mutually exclusive, thus determining M or not M is sufficient (i.e. $\mathcal{Q} = \{ M, \neg M \}$). This leaves us with a binary problem, represented with $y_i \in \{ 1, 0 \}$ where $y_i = 1$ implies M and $y_i = 0$ not M.   
