...
\subsection{Gradient Descent Methods}
    ...
    \subsection{Neural Networks}
        ...
        \subsubsection{Feed Forward}
            ...
            \begin{equation}
                a^l_i = f^l \pclosed{\sum_{j=1}^{N_{l-1}} w_{ij}^l a_j^{l-1} + b^l}
            \end{equation}  
...
\subsection{Classification}
    ...
    \subsubsection{Logistic regression}
        ...
        \end{align*}
        Where we have defined $p(x_i) \equiv p(\hat{y}_i = 1 | x_i, \vec{\theta})$, the probability that we guess $\hat{y}_i = 1$ given a data point $x_i$ and parameters $\vec{\theta}$. Solving the above expression for $p(x_i)$ yields the Sigmoid function from \cref{app:activation_functions} evaluated in $x_i^T \vec{\theta}$.
        ...
        \end{align*}
        The cost function then follows from taking logarithm of the likelihood, presented with an overall negative sign giving a minimisation problem of the parameters $\vec{\theta}$. This is in the literature often called the \textit{Binary Cross Entropy} (BCE) cost function \cite{BCE}.  

        \begin{align*}
            \msub{C}{BCE}(\vec{\theta}) &= -\log P(\vec{y}|X,\vec{\vec{\theta}}) \\ 
            &= - \sum_{i = 1}^{n} y_i \log[\sigma(X_{ia} \theta_a)] + (1 - y_i)\log[1-\sigma(X_{ia}\theta_a)]
        \end{align*}
        ...
        \begin{align*}
            \pdv{\theta_b} \msub{C}{BCE} &= - \sum_{i = 1}^{n} y_i (1-\sigma(X_{ia} \theta_a))X_{ib} - (1-y_i)\sigma(X_{ia} \theta_a)X_{ib} \\
            &= \sum_{i=1}^n (\sigma(X_{ia}\theta_a) - y_i)X_{ib}
        \end{align*}
        Finally, with $\vec{p}_i = \sigma(X_{ia}\theta_a)$
