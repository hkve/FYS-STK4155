    ...
    \section{Umbrella term for methods not neural networks}
        ...
        \subsection{Tree Based Methods}
            ...
            \subsubsection{Boosted trees}
                The idea behind boosted trees is combining several weak learners which iteratively learn from previous mistakes to enhance performance. In the case of decision trees, weak learners are shallow trees only performing slightly better than random guesses. 

                When employing gradient boost for regression the process starts by creating a single leaf, with a value $f_0(x)$ often set to the average of all output values in the training set. Implementing a differentiable cost function, e.g. the squared error for regression, the cost of the initial prediction, and its gradient with respect to the predicted value is calculated. In the case of squared error for regression, this results in the negative residual times 2. A shallow tree is fit to the negative gradient and a scaled version of that tree is added to the initial prediction, $f_0(x)$, creating the next: $f_1(x)$. The process of calculating the gradient of the cost function, fitting a new tree using the gradient, and adding this tree's prediction to the existing one, is repeated, iteratively creating a better model.      


        \subsection{Support Vector Machines}
        The Support Vector Machine (SVM) is most commonly used in classification problems, with the idea of constricting some hyperplane separating target classes in feature space \comment{Add ref}. 
        
        \begin{align*}
            \underset{\vec{w}}{\text{argmin}}&  \left[ \frac{1}{2}||\vec{w}||^2 \right], \\
            \text{subject to}\hspace{20px} &|y_i - \langle \vec{w}, x_i \rangle - b| \leq \epsilon  
        \end{align*}
        $\epsilon$ free parameter, serves a threshold ($\epsilon$ intensive tube), all predictions are in an $\epsilon$ range around the data point $y_i$ ~\citep{SVR}
        \begin{figure}[H]
            \centering
