...
\subsection{Datasets}
    ...
    \subsubsection{Premier League 2019 Season}
        \comment{Outline: (1) data acquisition + credits (2) main features/groups of features (3) assumptions and weaknesses -\Nanna}
        \comment{Some cool intro sentence. The following is very draft-y. Will write it properly when my head is clear xD -\Nanna} 
        As well as for many other sports, data science is becoming more and more leading ("stadig mer innflytelsesrik") in the football industry~\citep{Herbinet2018}, both from the viewpoint of the team (strategy, tactics, player acquisition), the odds (betting?) market (match, table, odds calculation) and the solely economically interested/motivated parties. Analysts working in the larger leagues have access to technology that allows them to analyse more complex data than the main match events such as number of goals scored or corner given. For instance, a group on football analysts will most likely measure a team's or player's performance through metrics like expected goals (xG) or the number of passes allowed per defensive action (PPDA). The former is a very much used measure in football, but the algorithm behind it is not universal. In the European Premier League (EPL), the xG for two teams is updated live by the minute during their clash. 
        
        For the biggest\comment{(most famous? coolest? best? idk -\Nanna)} football league in the world, the bookmakers keep the most interesting data to themselves. It is also common for a team to keep their database private. 

        The challenges in collecting data and the time scope of this project (has) compelled us to keep the purpose simple and ambitions somewhat low. In particular, we aim to foresee the outcome of a league match in EPL season 19/20 with a data set as shown in \comment{some figure}. 
        
        The choice to look back only one league game for one team is due to a great ease in the data acquisition process.  
