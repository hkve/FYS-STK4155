...
\subsection{Datasets}
    ...
    \subsubsection{Premier League 2019 Season}
        ...
        \comment{Some cool intro sentence. The following is very draft-y. -\Nanna} 
        As well as for many other sports, data science is becoming more and more leading ("stadig mer inflytelsesrik") in the football industry \citep{Herbinet2018}, both from the viewpoint of the team (strategy, tactics, player acquisition), the odds (betting?) market (match, table, odds calculation) and the economically interested/motivated parties. Analysts working in the larger leagues have technology at their disposal that allows them to analyse more complex data than the main match events such as number of goals scored or corner given. For instance, a group on football analysts will most likely measure a team's or player's performance through metrics like expected goals (xG) or the number of passes allowed per defensive action (PPDA). The former is a very much used measure in football, but the algorithm behind it is not universal. In the European Premier League (EPL), the xG of two teams is updated by the minute during their clash. 
        
        For the biggest\comment{(most famous? coolest? best? idk -\Nanna)} football league in the world, the bookmakers keep the most interesting data to themselves. It is also common for a team to keep their database private. To find a ...
