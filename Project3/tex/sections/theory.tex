...
\subsection{Neural Networks}
    ...
    When passing the output from one layer to the next, a kernel transformation is done. Denoting the output vector from layer $\ell$ as $\vec{a}^\ell$, the transformation produces $\vec{z}^{\ell+1} = W^\ell \vec{a}^\ell + \vec{b}^\ell$, where $W$ is a weight matrix, and $\vec{b}$ is called the bias. 
